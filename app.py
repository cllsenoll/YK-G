...
if uploaded_file is not None:
    try:
        ...
        # 3. F4 Ödeme Listesi Kontrolü
        elif "MUSTERI" in cols_str or "FIRMA" in cols_str or "FATURA" in cols_str or "F4" in fname_upper:
...
# ==========================================
# TAB 4: F4 ÖDEME LİSTESİ (ÖZEL ENTEGRASYON)
# ==========================================
    ...
    if st.session_state.f4_df is not None and not st.session_state.f4_df.empty:
        ...
        # Tüm olası personelleri belirleyelim (Öncelikli liste + eşleşmişler)
        ...
        # Seçilen personele ait otomatik eşleşen müşteriler
        ...
        if unmatched_df is not None and not unmatched_df.empty:
            ...
            for idx, row in unmatched_df.iterrows():
                ...
                if cols[2].button("➕ Ekle", key=f"add_unmatched_{idx}"):
                    # Yeni satırı seçili personelin manuel listesine ekle
                    ...
                    # Tanımsız listeden bu firmayı çıkar
        ...
        st.info("💡 Sol menüden **F4 ÖDEME LİSTESİ** dosyanızı yükleyin. Yüklenen veriler yalnızca bu panele özel olarak işlenecektir.")
